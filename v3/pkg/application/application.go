...
func (a *App) handleWindowMessage(event *windowMessage) {
	...
	// Check if the message starts with "wails:", "browser:data:", or "inspector-data:"
	if strings.HasPrefix(event.message, "wails:") || strings.HasPrefix(event.message, "browser:data:") || strings.HasPrefix(event.message, "inspector-data:") {
		a.info("handleWindowMessage: Processing wails/browser/inspector message", "message", event.message)
